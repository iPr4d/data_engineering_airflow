...
def store_data_mongo(**kwargs):
    ...
    # get data from XComs
    list_tweets_to_add = ti.xcom_pull(key='cleaned_data', task_ids='remove_existing_rows_'+subject)
...
def check_inserted_values(**kwargs):
    ...
    for subject in subjects:
        my_sql_inserted_count = ti.xcom_pull(key='return_value', task_ids='store_data_mysql_'+subject)
        mongo_inserted_count = ti.xcom_pull(key='return_value', task_ids='store_data_mongo_'+subject)
        if my_sql_inserted_count != mongo_inserted_count:
            raise ValueError('Inserted values are not the same in both services MySQL and MongoDB')
...
for subject in subjects :
    ...
    store_data_mysql_operator = PythonOperator(task_id='store_data_mysql_'+subject.replace(' ','_'),
                                               ...
                                               op_kwargs={'subject': subject.replace(' ','_')},
                                               dag=dag)
